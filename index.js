...
function addListeners() {
    const { fadeIn, fadeOut, move, scale, moveAndHide, showAndHide, heartBeating } = animaster()
    ...
    document.getElementById("backgroundColorPlay").addEventListener("click", function () {
        const block = document.getElementById("backgroundColorBlock")

        animaster().addBackgroundColor(400, "black").addBackgroundColor(400, "#8d3f8d").play(block)
    })

    document.getElementById("customAnimationPlay").addEventListener("click", function () {
        ...
        const test2 = animaster().addHeartBeating(1000)

        const a = animaster().addMove(111, { x: 10, y: -10 })
        const b = a.addFadeOut(400)

        const controls = a.play(block)

        block.stop = controls.stop
        block.reset = controls.reset
    })

    document.getElementById("customAnimationStop").addEventListener("click", function () {
        const block = document.getElementById("customAnimationBlock")
        block.stop()
    })

    document.getElementById("customAnimationReset").addEventListener("click", function () {
        const block = document.getElementById("customAnimationBlock")
        block.reset()
    })

    const animationHandler = animaster()
        .addBackgroundColor(500, "black")
        .addScale(300, 1.5)
        .addScale(300, 1)
        .addBackgroundColor(500, "#8d3f8d")
        .buildHandler()

    document.getElementById("customAnimationBlock").addEventListener("click", animationHandler)
}
